# CHERRY_KNOWLEDGE.md
## CHERRY WASH & DRY POIPET 24HR ΓÇö Customer AI Knowledge Base

**Source:** V3 locked rules (`PROJECT_MEMORY.md`, `V3_REWARD_RULE_FINAL_LOCK.md`, `app.py` customer copy)  
**Purpose:** Separate customer-support bot (own Render service) ΓÇö answer FAQs only; no billing/system actions  
...
**Pickup & delivery service hours:** 09:30 ΓÇô 00:00 (local time, UTC+7)  
Orders placed between 00:00ΓÇô09:29 are queued; staff start pickups from 09:30.
...
Pricing is **per machine package only** ΓÇö not per kg, not per piece, not by clothing quantity.

Approximation: **1 CHERRY laundry basket Γëê 1 small (14 kg) machine load**

### 14 kg Standard Package ΓÇö 210 Baht
...
### 14 kg Premium Package ΓÇö 240 Baht
...
### 18 kg Package + Extra Dry ΓÇö 300 Baht
...
- Home pickup & delivery available (fee by distance ΓÇö see below)
...
Delivery fee and other fees are **separate line items** on the invoice ΓÇö they are **not** part of package pricing and do **not** earn reward points.
...
Pickup orders (Order ID): delivery fee comes from the order ΓÇö staff cannot override it on the bill.
...
**How to order pickup:**
1. Use the CHERRY bot menu ΓåÆ Express Pickup
...
**Payment before service** ΓÇö pay after staff check items and issue invoice, or cash at pickup.

---

## Estimated Ready Time

**Pickup & delivery orders:** approximately **3ΓÇô4 hours** after:
- Laundry is collected
- Staff have checked items
- Invoice is issued

**Walk-in (customer brings to shop):** approximately **2ΓÇô3 hours** after staff check and issue invoice.

**Orders collected after 20:00:** shop tries same-night delivery if workload allows; otherwise next day from 09:30.

Times are **estimates** ΓÇö may vary by volume, weather, power outages, and daily order load.
...
- **Cash** ΓÇö pay staff at pickup, or leave in laundry bag
- **Bank transfer after invoice** ΓÇö staff send invoice and total after checking items

Accepted transfer channels: **ABA ΓÇó ACLEDA ΓÇó INDO BANK ΓÇó THAI BANK**
...
Points are calculated **per eligible package line** ΓÇö not by invoice total.
...
- 210 Baht tier ΓÇö even if total bill exceeds 240 Baht

**Example:** 14 kg (210) ├ù 2 + 70 B delivery = **0 points** (not 1).

**When points apply:**

| Order type | Requirement |
|------------|-------------|
| Walk-in (at shop) | Valid Member ID (`CWD-xxx`) ΓÇö **no `/start` required** for points at store billing |
| Pickup (Order ID) | Customer must press **`/start`** on the bot (`BOT_STARTED=YES`) |

To check points anytime: use **≡ƒÄü Rewards / Member** menu in the main CHERRY bot.
...
| When credit applies | On the **NEXT laundry invoice after** reaching 13/13 ΓÇö **not** on the invoice that completes 13 points |
...
### Before washing ΓÇö customer must inform staff if laundry contains
...
Walk-in customer bills show Member ID ΓÇö Order ID is not shown on walk-in customer invoice.  
Pickup orders use Order ID for tracking in the bot.

---

## AI Support Bot ΓÇö Allowed Actions
...
## AI Support Bot ΓÇö Prohibited Actions
...
- Change reward rules
- Guess order status without Order ID ΓÇö direct customer to main bot **My Order** or staff
...
> α╕éα╕¡α╕Üα╕äα╕╕α╕ôα╕ùα╕╡α╣êα╕òα╕┤α╕öα╕òα╣êα╕¡ CHERRY Wash & Dry α╕äα╣êα╕░ α╕ùα╕╡α╕íα╕çα╕▓α╕Öα╕êα╕░α╕èα╣êα╕ºα╕óα╣Çα╕½α╕Ñα╕╖α╕¡α╕äα╕╕α╕ôα╣âα╕Öα╣äα╕íα╣êα╕èα╣ëα╕▓

---

## Common Customer Questions ΓÇö Quick Answers

**Are you open?**  
Yes ΓÇö shop is open **24 hours**. Pickup/delivery runs **09:30ΓÇô00:00**.

**How much for one bag?**  
Depends on machine size: small 14 kg **210ΓÇô240 B**, large 18 kg **270ΓÇô300 B** per machine ΓÇö not by weight.

**I only have a little / sedikit / not much laundry ΓÇö is it cheaper?**  
No ΓÇö CHERRY charges **per machine package**, not by clothing amount. Even a small load uses one full machine: minimum **210 B** (14 kg standard). Options: 14 kg 210/240 B, 18 kg 270/300 B.

**Do I get points on 210 Baht package?**  
No ΓÇö only **240 / 270 / 300** tiers earn **+1 point per machine**.

**Do delivery fees give points?**  
No ΓÇö points are from eligible packages only.

**When do I get the 200 Baht credit?**  
After reaching **13/13 points**, credit applies on your **next** laundry order ΓÇö not the order that hit 13.

**Do you iron clothes?**  
No ironing service.

**How long until ready?**  
Walk-in ~2ΓÇô3 hours; pickup/delivery ~3ΓÇô4 hours after collection and invoice ΓÇö estimates only.

**How do I track my order?**  
Use the main `@CherryWashDryPoipetBot` ΓåÆ **My Order** menu, or contact staff with your Order ID.
...
| Separate support bot (planned) | FAQ + policy answers only ΓÇö read this file |

Do not duplicate billing logic in the support bot. When customer needs invoice, points update, or delivery action ΓåÆ direct them to main bot or staff contact channels above.
